...
city.addEventListener('change',function(e){
  ...
  if( e.target.value == "" ) {
    ...
    pharmacyList = data.filter((item) => {
      if( item.properties.address.indexOf( e.target.value ) > -1 ){
        alllatLng.push(L.latLng(item.geometry.coordinates[1], item.geometry.coordinates[0]));
        return item;
      } // if
    });
  ...
  moveToSelectMarker( alllatLng );
});


dist.addEventListener('change',function(e){
  search.value = "";
  let cityDist = city.value + e.target.value;
  let pharmacyList = [];
  let alllatLng =[];
  pharmacyList = data.filter((item) => {
    if( item.properties.address.indexOf( cityDist ) > -1 ){
      alllatLng.push(L.latLng(item.geometry.coordinates[1], item.geometry.coordinates[0]));
      return item;
    } // if
  });
...
searchIcon.addEventListener('click',function(){
  ...
  if( searchAddr == "" ){
    ...
    let alllatLng =[];
    pharmacyList = data.filter((item) => {
      if( item.properties.address.indexOf( searchAddr ) > -1 ){
        alllatLng.push(L.latLng(item.geometry.coordinates[1], item.geometry.coordinates[0]));
        return item;
      } // if
    });
...
list.addEventListener('click',function(e){
  ...
  if( e.target.nodeName !== "A" ){
    ...
    e.preventDefault();
    var latlng = L.latLng( e.target.dataset.lat, e.target.dataset.lng );
...
sideBtn.addEventListener('click',silde);
...
function getData(){
  ...
  xhr.onload = function(){
    ...
    originData.features.forEach((element) => data.push(element));
...
function showPharmacyList( pharmacyList ){
  let listH = window.innerHeight - document.querySelector(".searchDiv").offsetHeight;
  list.style.height = listH + "px";
  
  let str = "";
  if( pharmacyList.length === 0 ){ // 初始
    ...
    document.querySelector(".listLoading").style.display = "block";
    emptyHint.style.display = "none";
    let adultMask = "";
    let childMask = "";
    pharmacyList.forEach((item) => {
      adultMask = item.properties.mask_adult !== 0 ? "btnBlue" : "btnGray";
      childMask = item.properties.mask_child !== 0 ? "btnOrange" : "btnGray";
    
      str += `<li>
                <div class="title">
                  <h2 class="name">${item.properties.name}</h2>
                  <a href="#" class="pharMarker fas fa-map-marker" data-lat="${item.geometry.coordinates[1]}" data-lng="${item.geometry.coordinates[0]}"></a>
                </div>
                ...
                <div class="maskStock">
                  <div class="adult ${adultMask}">
                    <span>成人口罩</span>
                    <span>${item.properties.mask_adult}</span>
                  </div>
                  <div class="child ${childMask}">
                    <span>孩童口罩</span>
  ...
  document.querySelector(".listLoading").style.display = "none";
  list.innerHTML = str;

} // showPharmacyList()

function updateCity( selectCity ){
  let str = "" ;
  
 
  str += `<option value="">請選擇縣市</option>`;
  area.forEach((element) =>
  str += `<option value="${element.CityName}">${element.CityName}</option>` ) ;
  ...
  updateDist( selectCity )

} // updateCity()

function updateDist( selectCity ){
  let str = "" ;

  if( selectCity === "" ){
    str += `<option value="">全部地區</option>`
  } // if

  else {
    const select = area.find(function(item){
      return item.CityName === selectCity ; } );         // 取得所選縣市所屬地區

    str += `<option value="">全部地區</option>`
    select.AreaList.forEach( (element) => 
...
function buildMap(){
  map = L.map('map').setView([25.1227609, 121.4632199], 18);
...
function onLocationFound(e) {
  ...
  data.filter((item) =>{
    ...
    if( myLocation.distanceTo( latlng ) <= 500 ) 
...
function addMarker(){
  ...
  for( let i = 0 ; i < data.length ; i++){
    adultMask = data[i].properties.mask_adult !== 0 ? "btnBlue" : "btnGray";
    childMask = data[i].properties.mask_child !== 0 ? "btnOrange" : "btnGray";
    ...
    marker = L.marker([data[i].geometry.coordinates[1],data[i].geometry.coordinates[0]], {icon: iconColor})
             .bindPopup(`<div class="popInfo">
                          <div class="title">
                            <h2 class="name">${data[i].properties.name}</h2>
                          </div>
                          
                          <div class="info">
                            <span>${data[i].properties.address}</span>
...
function moveToSelectMarker( alllatLng ){
...
function silde(){
  document.getElementById("map").classList.toggle("transform");
  document.querySelector(".sidebar").classList.toggle("transform");
  document.querySelector(".sidebar i").classList.toggle("fa-rotate-180");
